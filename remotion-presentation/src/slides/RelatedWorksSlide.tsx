import React from 'react';
import { interpolate, useCurrentFrame } from 'remotion';
import { SlideLayout } from '../components/SlideLayout';
import { theme } from '../styles/theme';

interface RelatedWork {
  title: string;
  description: string;
  citation?: string;
}

interface RelatedWorksSlideProps {
  presenterName?: string;
  works: RelatedWork[];
  footerTitle: string;
  pageNumber: number;
}

export const RelatedWorksSlide: React.FC<RelatedWorksSlideProps> = ({
  presenterName,
  works,
  footerTitle,
  pageNumber,
}) => {
  const frame = useCurrentFrame();

  return (
    <SlideLayout footerTitle={footerTitle} pageNumber={pageNumber} animate={false}>
      <h1
        style={{
          fontSize: theme.fontSize.heading - 20,
          fontWeight: 'bold',
          color: theme.colors.text,
          margin: 0,
          marginBottom: presenterName ? theme.spacing.xs : theme.spacing.md,
        }}
      >
        (iii) Related Works/Literature Review
      </h1>
      {presenterName && (
        <p
          style={{
            fontSize: theme.fontSize.small,
            color: theme.colors.textMuted,
            margin: 0,
            marginBottom: theme.spacing.md,
          }}
        >
          Presented by: {presenterName}
        </p>
      )}

      <div style={{ display: 'flex', flexDirection: 'column', gap: theme.spacing.sm }}>
        {works.map((work, index) => {
          const itemOpacity = interpolate(
            frame,
            [10 + index * 10, 25 + index * 10],
            [0, 1],
            { extrapolateRight: 'clamp' }
          );

          return (
            <div
              key={index}
              style={{
                backgroundColor: '#f8f9fa',
                borderLeft: `4px solid ${theme.colors.primary}`,
                padding: theme.spacing.sm,
                opacity: itemOpacity,
              }}
            >
              <h3
                style={{
                  fontSize: theme.fontSize.body - 6,
                  fontWeight: 'bold',
                  color: theme.colors.primary,
                  margin: 0,
                  marginBottom: theme.spacing.xs / 2,
                }}
              >
                {work.title} {work.citation && <span style={{ fontWeight: 'normal' }}>{work.citation}</span>}
              </h3>
              <p
                style={{
                  fontSize: theme.fontSize.small - 2,
                  color: theme.colors.textLight,
                  margin: 0,
                  lineHeight: 1.3,
                }}
              >
                {work.description}
              </p>
            </div>
          );
        })}
      </div>
    </SlideLayout>
  );
};
